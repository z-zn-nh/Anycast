...
> ---
>
> **2026-09-24 修订（二）** —— 索引维护层落地，按实测重写若干描述：
>
> | 条目 | 修订内容 |
> | :--- | :--- |
> | §9 / 架构图 | 索引增量由 `USN Journal` 改为 **目录 mtime 增量对账**（USN 需管理员权限，与绿色免安装冲突） |
> | §9 | 明确**不再每次启动全量重扫**，改为「mtime 未变的目录整棵跳过」 |
> | §25 | 索引项补充：`parent` / `mtime_ns` 两列、`content_meta` 表、一致性校验与空间回收 |
> | §26 | Phase 1 索引维护**已完成**（含 5 处实现缺陷修复），详见开发文档 §4.1.2 / §4.3.4 |
> | 新增 | 命令行维护入口：`--index-status` / `--index-compact` / `--index-scan` |
>
> 实测依据：本机 `anycast.db` 曾出现「正文索引 543 条、文件索引仅 1 条」的严重不一致，
> 根因是「重建索引」在启动扫描进行中清空索引却跳过重扫。完整推导见开发文档 §4.1.2。
...
> | 行为 | 要求 |
> | :--- | :--- |
> | **唤起** | 全局热键（默认 `Ctrl+Alt+Q`）+ 托盘图标。**必须在任意前台页面上都能唤起**，不依赖当前焦点所在程序 |
...
General (常规设置)
├── Global Wake Hotkey (唤醒搜索窗口，默认 Ctrl+Alt+Q)
